...
class WSClient {
	...
	*/
	...
	/**
	 * Do Translate Operation
	 */
	function doTranslate($totranslate, $language, $module) {
		// Perform re-login if required.
		$this->__checkLogin();
		...
		$postdata = Array(
			...
			'sessionName' => $this->_sessionid,
			'totranslate' => $this->toJSONString($totranslate),
			'language' => $language,
			'module' => $module,
		);
		$resultdata = $this->_client->doPost($postdata, true);
	...
	/**
	 * Get Filter fields Operation
	 */
	function doGetFilterFields($module) {
		// Perform re-login if required.
		$this->__checkLogin();
