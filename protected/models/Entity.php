...
class Entity extends EActiveResource
{
	 ...
	 */
	public static function model($module,$getFields=true)
	{
		$obj=parent::model(__CLASS__);
		$obj->init($module,$getFields);
		return $obj; 
	}
    /**
     ...
     */
    public function init($module,$getFields=true)
    {
    	$this->setModule($module);
    	if($getFields) $this->getFields();
    }
...
//		return array(
//			'account' => array(self::BELONGS_TO, 'Crmentity', 'accountid'),
//			'accountbillads' => array(self::HAS_ONE, 'Accountbillads', 'accountaddressid'),
//			'accountscf' => array(self::HAS_ONE, 'Accountscf', 'accountid'),
//			'accountshipads' => array(self::HAS_ONE, 'Accountshipads', 'accountaddressid'),
//		);
//	}
	 ...
	 */
	public function search($pgsize=null,$action='browse')
	{
		...
		return new CActiveDataProvider($this, array(
			'criteria'=>$criteria,
			'pagination'=>array(
				'pageSize'=>(!is_null($pgsize) ? $pgsize : Yii::app()->user->getState('pageSize',Yii::app()->params['defaultPageSize'])),
				'route'=>'#entity/' . $this->getModule() . "/$action/",
			),
			'sort'=>array(
				'route'=>'#entity/' . $this->getModule() . "/$action/",
			),
	...
	public function delete($identifier)
	{
		...
		$sql = 'DELETE FROM ' . self::$db->quoteTableName(Yii::app()->getRequest()->getParam('module')) . ' WHERE ';
	...
	{
		$model=Entity::model($this->getModule());
		$model->findById($id);
                $dvfields=array();
		if (!empty($this->_attributes))
			$fields=$this->_attributes;
	...
	{		
		...
		switch ($uitype) {
			...
			case 101:				
				$widget=array(
				 'name'=>$fieldname,
				 'value'=>'Entity::getComplexAttributeValue("'.$fieldname.'",$data["'.$fieldname.'"])',
				);
				break;
			case 15:
			case 16:
	...
	function getViewLinkField($fieldname) {
		...
		return 'CHtml::link(CHtml::encode($data["'.$fieldname.'"]),"#entity/'.$this->getModule().'/view/id/".$data["id"]."/Entity_page/".('.$gridOffset.'+$row))';
	}
}
